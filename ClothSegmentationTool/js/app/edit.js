...
function(Layer, Annotator, util) {
  ...
  // Download trick.
  function downloadURIForLocal(uri, filename) {
	...
	function downloadURI(uri, filename) {
		...
		// var albumBucketName = 'jsannotator';
		var albumBucketName = 'js-image-annotator';
		// Initialize the Amazon Cognito credentials provider
		AWS.config.region = 'ap-south-1'; // Region
		AWS.config.credentials = new AWS.CognitoIdentityCredentials({
		IdentityPoolId: 'ap-south-1:bc2c3fc3-8555-4422-bd3e-723a3afea762',
		// IdentityPoolId: 'ap-south-1:7e263142-3cce-47b3-9a62-173c09ec9309'
		...
		var imgContent = base64toBlob(uri.split(',')[1], 'image/png');
		
		var photoKey = 'js-segment-annotator-master/data/annotations/' + filename;
		// var photoKey = 'js-annotator-enhancement/data/annotations/' + filename;
  ...
  function render(data, params) {
    ...
    var annotator = new Annotator(data.imageURLs[id], {
          ...
          onload: function () {
            // Calling Bounding Box Creation: I319452
			annotator.drawBoundingBox(data.imageURLs[id]);
