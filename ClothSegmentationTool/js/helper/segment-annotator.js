...
function (Layer, segmentation, morph) {
  ...
  // Drawing Bounding Boxes for every Object: I319452
  Annotator.prototype.drawBoundingBox = function (imageName, options) {
    console.log('Bounding Box Drawing Started...');
	var ctx = this.layers.boundingbox.canvas.getContext('2d');
	ctx.globalAlpha = 0.3;
	ctx.strokeStyle = "black";
	
	// Below 4 lines assume image file names have specific format like "image1_21_23_110_120.png" i.e. last 4 numbers are the x_min, y_min, x_max, y_max of the bounding box of the object in the image.
	
	var xmin = parseInt(imageName.split('.')[0].split('_').reverse()[3])
	var ymin = parseInt(imageName.split('.')[0].split('_').reverse()[2])
	var xmax = parseInt(imageName.split('.')[0].split('_').reverse()[1])
	var ymax = parseInt(imageName.split('.')[0].split('_').reverse()[0])
	dispX = xmax-xmin;
	dispY = ymax-ymin;
	
    // ctx.rect(boundingBoxPoints.xmin,boundingBoxPoints.ymin,boundingBoxPoints.xmax,boundingBoxPoints.ymax); // E.g. ctx.rect(80,6,127,124);
	ctx.rect(xmin, ymin, dispX, dispY);
	ctx.fillStyle = "#8ED6FF";
	ctx.fill();
	ctx.globalAlpha = 1.0;
	ctx.stroke();
	console.log('Bounding Box Drawing Done!');
